...
//#define CoefLuma float3(1.0/3.0, 1.0/3.0, 1.0/3.0) // Equal weight coefficient
  ...
  /                          Main code                          /
  '-----------------------------------------------------------*/
float4 LumaSharpenPass(in Texture2D<float4> tex, in float2 uv, in float2 inverseTextureSize){
	// -- Get the original pixel --
	float4 ori4 = tex2D(tex, uv); // ori = original pixel
	float3 ori = ori4.rgb; // ori = original pixel
	...
	if (pattern == 0){
		...
		//   [ 2/9, 8/9, 2/9]  =  [ 2 , 8 , 2 ]
		//   [    , 2/9, 1/9]     [   , 2 , 1 ]
		blur_ori  = tex2D(tex, uv + (inverseTextureSize / 3.0) * offset_bias).rgb;  // North West
		blur_ori += tex2D(tex, uv + (-inverseTextureSize / 3.0) * offset_bias).rgb; // South East
		//blur_ori += tex2D(tex, uv + (inverseTextureSize / 3.0) * offset_bias); // North East
		//blur_ori += tex2D(tex, uv + (-inverseTextureSize / 3.0) * offset_bias); // South West
	...
	if (pattern == 1){
		...
		//   [ .50,   1, .50]  =  [ 2 , 4 , 2 ]
		//   [ .25, .50, .25]     [ 1 , 2 , 1 ]
		blur_ori  = tex2D(tex, uv + float2(inverseTextureSize.x, -inverseTextureSize.y) * 0.5 * offset_bias).rgb; // South East
		blur_ori += tex2D(tex, uv - inverseTextureSize * 0.5 * offset_bias).rgb;  // South West
		blur_ori += tex2D(tex, uv + inverseTextureSize * 0.5 * offset_bias).rgb; // North East
		blur_ori += tex2D(tex, uv - float2(inverseTextureSize.x, -inverseTextureSize.y) * 0.5 * offset_bias).rgb; // North West
	...
	if (pattern == 2){
		...
		//   [ 4 ,16 ,24 ,16 ,   ]
		//   [   ,   , 6 , 4 ,   ]
		blur_ori  = tex2D(tex, uv + inverseTextureSize * float2(0.4, -1.2) * offset_bias).rgb;  // South South East
		blur_ori += tex2D(tex, uv - inverseTextureSize * float2(1.2, 0.4) * offset_bias).rgb; // West South West
		blur_ori += tex2D(tex, uv + inverseTextureSize * float2(1.2, 0.4) * offset_bias).rgb; // East North East
		blur_ori += tex2D(tex, uv - inverseTextureSize * float2(0.4, -1.2) * offset_bias).rgb; // North North West
		blur_ori *= 0.25;  // ( /= 4) Divide by the number of texture fetches
	...
	if (pattern == 3){
		...
		//   [ .50,    , .50]  =  [ 1 ,   , 1 ]
		//   [ .50, .50, .50]     [ 1 , 1 , 1 ]
		blur_ori  = tex2D(tex, uv + float2(0.5 * inverseTextureSize.x, -inverseTextureSize.y * offset_bias)).rgb;  // South South East
		blur_ori += tex2D(tex, uv + float2(offset_bias * -inverseTextureSize.x, 0.5 * -inverseTextureSize.y)).rgb; // West South West
		blur_ori += tex2D(tex, uv + float2(offset_bias * inverseTextureSize.x, 0.5 * inverseTextureSize.y)).rgb; // East North East
		blur_ori += tex2D(tex, uv + float2(0.5 * -inverseTextureSize.x, inverseTextureSize.y * offset_bias)).rgb; // North North West
	...
	return float4(outputcolor, ori4.a);
}
#endif
