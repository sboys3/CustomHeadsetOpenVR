// Taken from https://github.com/Rectus/openvr_camera_sim/blob/main/camera_component.cpp
// Added a couple of helpers at the bottom.
...
namespace vr {
    ...
    class IVRPaths {
      public:
        ...
        virtual ETrackedPropertyError HandleToString(PathHandle_t pHandle,
    ...
    class IVRBlockQueue {
      public:
        virtual EBlockQueueError Create(PropertyContainerHandle_t* pulQueueHandle,
                                        const char* pchPath,
        ...
        virtual EBlockQueueError AcquireWriteOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                                                       PropertyContainerHandle_t* pulBlockHandle,
                                                       void** ppvBuffer) = 0;

        virtual EBlockQueueError ReleaseWriteOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                                                       PropertyContainerHandle_t ulBlockHandle) = 0;
        ...
        virtual EBlockQueueError AcquireReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                                                      PropertyContainerHandle_t* pulBlockHandle,
                                                      void** ppvBuffer,
                                                      EBlockQueueReadType eReadType) = 0;

        virtual EBlockQueueError ReleaseReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                                                      PropertyContainerHandle_t ulBlockHandle) = 0;
    ...
    static IVRPaths* _VRPaths() {
        ...
        if (pPaths == nullptr) {
            EVRInitError eError;
            pPaths = (IVRPaths*)VRDriverContext()->GetGenericInterface(IVRPaths_Version, &eError);
        }
        return pPaths;
    }
    ...
    static IVRBlockQueue* _VRBlockQueue() {
        ...
        if (pBlockQueue == nullptr) {
            EVRInitError eError;
            pBlockQueue = (IVRBlockQueue*)VRDriverContext()->GetGenericInterface(IVRBlockQueue_Version, &eError);
        }
        return pBlockQueue;
    }

    inline IVRPaths* VRPaths() {
        return _VRPaths();
    }
    inline IVRBlockQueue* VRBlockQueue() {
        return _VRBlockQueue();
    }

    typedef const char* CVRPathHandle;

    static const CVRPathHandle k_pathCameraWidth("/width");
    static const CVRPathHandle k_pathCameraHeight("/height");
    static const CVRPathHandle k_pathCameraFormat("/format");
    static const CVRPathHandle k_pathCameraFrameSequence("/frame_sequence");
    static const CVRPathHandle k_pathCameraFrameSize("/frame_size");
    static const CVRPathHandle k_pathCameraFrameTimeMonotonic("/frame_time_monotonic");
    static const CVRPathHandle k_pathCameraFrameServerTimeTicks("/server_time_ticks");
    static const CVRPathHandle k_pathCameraFrameDeliveryRate("/delivery_rate");
    static const CVRPathHandle k_pathCameraFrameElapsedTime("/elapsed_time");

    template <typename T>
    ETrackedPropertyError
    VRPathsSet(PropertyContainerHandle_t ulRootHandle, CVRPathHandle pszPath, T value, PropertyTypeTag_t unTag) {
        PathWrite_t batch{};
        batch.writeType = PropertyWrite_Set;
        T typedValue = value;
        batch.pvBuffer = &typedValue;
        batch.unBufferSize = sizeof(T);
        batch.unTag = unTag;
        batch.pszPath = (char*)pszPath;

        ETrackedPropertyError error = VRPaths()->WritePathBatch(ulRootHandle, &batch, 1);
